...
class MultiScaleTemporal1D(nn.Module):
    """
    DTANet
    - 并联若干 depthwise 1D 时域卷积分支 + 1x1 pointwise 混合电极
    - 全局时域池化 + 线性映射到 out_dim
    ...
    def __init__(self, in_channels, out_dim=64,
        ...
        per_branch = max(16, out_dim // num_br)  # 每分支通道数

        branches = []
        for k, d in zip(kernel_sizes, dilations):
            pad = d * (k // 2)
            branch = nn.Sequential(
                # depthwise: 沿时间卷积，不混合电极
                nn.Conv1d(in_channels, in_channels, kernel_size=k, dilation=d,
        ...
        # 对融合后的通道做 ECA
        self.eca_after = ECA1D(channels=per_branch * num_br, k_size=3, dropout=dropout)

        self.pool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Linear(per_branch * num_br, out_dim)
    ...
    def forward(self, x):
        ...
        for br in self.branches:
            outs.append(br(x))         # 每个: (B, per_branch, T)
        y = torch.cat(outs, dim=1)     # (B, per_branch*num_br, T)
        y = self.eca_after(y)          # (B, F, T)
        y = self.pool(y).squeeze(-1)   # (B, F)
