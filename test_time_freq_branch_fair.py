...
from Models.PyTorchModels_backup_before_lightweight import FeaAndEmg_se_PyTorch, LinearResidualBlock, EMA

class FairTimeFreqModel(nn.Module):
    """公平对比模型：使用相同的空间分支，只改变时频分支"""
    def __init__(self, emg_channels, time_steps, feature_dim, num_classes, dropout_rate=0.4):
        super().__init__()
        # 空间分支：与原始模型相同
        self.space_branch = nn.Sequential(
            EMA(emg_channels, time_steps),
            nn.Linear(64, 128),
            nn.BatchNorm1d(128),
            nn.PReLU()
        )
...
def test_fair_comparison():
    ...
    train_dataset = FairDataset(emg_train, feature_train, label_train)
    val_dataset = FairDataset(emg_val, feature_val, label_val)
    ...
    models = {
        ...
        'without_time_freq_branch': FairTimeFreqModel(12, 400, 64, np.max(label_train)+1)
    }
